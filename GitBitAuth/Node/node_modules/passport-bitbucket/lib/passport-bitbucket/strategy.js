...
Strategy.prototype.getRepos = function (token, tokenSecret, params, done) {
    this._oauth.get('https://bitbucket.org/api/1.0/user/repositories/', token, tokenSecret, function (err, body, res) {
        if (err) { return done(new InternalOAuthError('failed to fetch user profile', err)); }
